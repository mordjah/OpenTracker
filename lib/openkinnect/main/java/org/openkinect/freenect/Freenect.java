...
public class Freenect implements Library {
	...
	static {
		try {
			...
			System.err.println( "Loaded " + instance.getName() + " from " +
								instance.getFile().getCanonicalPath() );
	...
	public static NativeContext createNativeContext () {
		PointerByReference ctxPtr = new PointerByReference();
		int rval = freenect_init( ctxPtr, Pointer.NULL );
		if ( rval == 0 ) {
			NativeContext ctx = new NativeContext( ctxPtr.getValue() );
	...
	public static class NativeContext extends PointerType implements Context {
		...
		public final NativeLogCallback logCallback =
		...
		public NativeContext ( Pointer ptr ) {
			super( ptr );
		}
		...
		public NativeDevice openNativeDevice ( int index ) {
			PointerByReference devicePtr = new PointerByReference();
			int rval = freenect_open_device( this, devicePtr, index );
			if ( rval != 0 ) {
				throw new IllegalStateException(
	...
	public static class NativeDevice extends PointerType implements Device {

		public FrameMode videoMode;
		public ByteBuffer videoBuffer;
		public VideoHandler videoHandler;
		public FrameMode depthMode;
		public ByteBuffer depthBuffer;
		public DepthHandler depthHandler;
		public final DoubleBuffer accelX = DoubleBuffer.allocate( 1 );
		public final DoubleBuffer accelY = DoubleBuffer.allocate( 1 );
		public final DoubleBuffer accelZ = DoubleBuffer.allocate( 1 );
		public int index;
		public TiltState rawTiltState;
		public TiltStatus tiltStatus;
		public double tiltAngle;
		public double[] accel;
		public final NativeVideoCallback videoCallback =
										 new NativeVideoCallback() {

			@Override
			public void callback ( Pointer dev, Pointer depth, int timestamp ) {
				videoHandler.
						onFrameReceived( videoMode, videoBuffer, timestamp );
			}

		};
		public final NativeDepthCallback depthCallback =
										 new NativeDepthCallback() {

			@Override
			public void callback ( Pointer dev, Pointer depth, int timestamp ) {
				depthHandler.
						onFrameReceived( depthMode, depthBuffer, timestamp );
			}
		...
		public NativeDevice ( Pointer ptr, int index ) {
			super( ptr );
			this.index = index;
		...
		public void setDepthFormat ( DepthFormat fmt, Resolution res ) {
			FrameMode.ByValue mode = freenect_find_depth_mode( res.intValue(),
															   fmt.intValue() );
			if ( mode.isValid() ) {
		...
		public void setVideoFormat ( VideoFormat fmt, Resolution res ) {
			FrameMode.ByValue mode = freenect_find_video_mode( res.intValue(),
															   fmt.intValue() );
			if ( mode.isValid() ) {
		...
		public void refreshTiltState () {
			...
			this.tiltStatus = TiltStatus.fromInt( freenect_get_tilt_status(
					rawTiltState ) );
		...
		public double[] getAccel () {
			return new double[] { accelX.get( 0 ), accelY.get( 0 ), accelZ.get(
				0 ) };
		}
	...
	public static class EventThread extends Thread {
		...
		public final NativeContext ctx;
	...
	public interface NativeDepthCallback extends Callback {

		void callback ( Pointer dev, Pointer depth, int timestamp );

	};

	public interface NativeVideoCallback extends Callback {

		void callback ( Pointer dev, Pointer frame, int timestamp );

	};

	public static native int freenect_init ( PointerByReference ctx,
											 Pointer usb_ctx );

	public static native int freenect_shutdown ( NativeContext ctx );

	public static native void freenect_set_log_level ( NativeContext ctx,
													   int level );

	public static native void freenect_set_log_callback ( NativeContext ctx,
														  NativeLogCallback cb );

	public static native int freenect_process_events ( NativeContext ctx );

	public static native int freenect_num_devices ( NativeContext ctx );

	public static native int freenect_open_device ( NativeContext ctx,
													PointerByReference dev,
													int index );

	public static native int freenect_close_device ( NativeDevice dev );

	public static native void freenect_set_user ( NativeDevice dev,
												  Pointer user );

	public static native Pointer freenect_get_user ( NativeDevice dev );

	public static native void freenect_set_depth_callback ( NativeDevice dev,
															NativeDepthCallback cb );

	public static native void freenect_set_video_callback ( NativeDevice dev,
															NativeVideoCallback cb );

	public static native int freenect_set_depth_buffer ( NativeDevice dev,
														 ByteBuffer buf );

	public static native int freenect_set_video_buffer ( NativeDevice dev,
														 ByteBuffer buf );

	public static native int freenect_start_depth ( NativeDevice dev );

	public static native int freenect_start_video ( NativeDevice dev );

	public static native int freenect_stop_depth ( NativeDevice dev );

	public static native int freenect_stop_video ( NativeDevice dev );

	public static native int freenect_update_tilt_state ( NativeDevice dev );

	public static native TiltState freenect_get_tilt_state ( NativeDevice dev );
	...
	public static native int freenect_set_tilt_degs ( NativeDevice dev,
													  double angle );

	public static native int freenect_set_led ( NativeDevice dev, int option );

	public static native void freenect_get_mks_accel ( TiltState tiltState,
													   DoubleBuffer x,
													   DoubleBuffer y,
													   DoubleBuffer z );

	public static native int freenect_select_subdevices ( NativeContext ctx,
														  int flag );

	public static native int freenect_get_video_mode_count ();

	public static native FrameMode.ByValue freenect_get_video_mode (
			int mode_num );

	public static native FrameMode.ByValue freenect_get_current_video_mode (
			NativeDevice dev );

	public static native FrameMode.ByValue freenect_find_video_mode ( int res,
																	  int fmt );

	public static native int freenect_set_video_mode ( NativeDevice dev,
													   FrameMode.ByValue mode );

	public static native int freenect_get_depth_mode_count ();

	public static native FrameMode.ByValue freenect_get_depth_mode (
			int mode_num );

	public static native FrameMode.ByValue freenect_get_current_depth_mode (
			NativeDevice dev );

	public static native FrameMode.ByValue freenect_find_depth_mode ( int res,
																	  int fmt );

	public static native int freenect_set_depth_mode ( NativeDevice dev,
													   FrameMode.ByValue mode );
